...
def copy_oper(app_type):
    ...
    os.chdir(HOME_DIR + '/../bin/' + APP_TYPE + '/')
    os.system(f'riscv64-linux-gnu-objdump -d {APP_STD_ELF} > prog.txt')
    os.system(f'riscv64-linux-gnu-objcopy -I binary -O verilog {APP_STD_BIN} prog.flash')
